...
create_cluster()
{
az aks create --resource-group "$resource_group" --name "$name" --subscription "$subscription" --tags env="$env" --kubernetes-version "$kubernetes_version"  --node-vm-size "$node_vm_size" --node-count "$node_count" --network-plugin "$network_plugin" --generate-ssh-keys --max-pods "$max_pods" --location "$location" --network-plugin "$network_plugin" --dns-name-prefix "$dns_name_prefix" --vnet-subnet-id "$vnet_subnet_id" --admin-username "$admin_username" --node-osdisk-size "$node_osdisk_size" --nodepool-name "$nodepool_name" --service-cidr "$service_cidr" --dns-service-ip "$dns_service_ip" --docker-bridge-address "$docker_bridge_address" --enable-private-cluster --disable-public-fqdn --outbound-type userDefinedRouting --load-balancer-sku standard
}
create_cluster
